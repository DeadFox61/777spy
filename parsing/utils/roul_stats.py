...
class RoulStats():
    def __init__(self, id, roul_id, name, nums = []):
        self.id = id
    ...
    def add_nums(self,nums):
        ...
        for num in nums:
            ...
            i+=1
        if i:
            print(f'{i} чисел добавлено в {self.roul_id}')
    def from_db(self):
        self.add_nums(db.get_curr(self.roul_id))
    def to_json(self):
        res = {}
...
class RoulsStats():
    ...
    def init_roul(self, id , roul_id, name):
        if roul_id not in self.rouls:
            roul = RoulStats(id = id, roul_id = roul_id, name = name)
            roul.from_db()
            self.rouls[roul_id] = roul
    def init_rouls(self, from_db = True, roul_data = None):
        if from_db:
            roul_data = db.get_roul_data()
        for roul in roul_data:
            self.init_roul(roul["id"] , roul["roul_id"],roul["name"])
